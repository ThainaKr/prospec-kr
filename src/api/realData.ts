...
export async function loadRealDataSnapshot(): Promise<RealDataSnapshot> {
  const [profiles, lists, contacts, recoveries, appointments, notifications, chips, templates] = await Promise.all([
  ...
  return { profiles, lists, contacts, recoveries, appointments, notifications, chips, templates };
}
